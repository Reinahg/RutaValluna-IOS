...
class LoginViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let str : NSString = "\(Localized.acceptPolicy)" as NSString
        ...
        let range1 : NSRange = str.range(of: Localized.termsOfService)
        labelTermOfService.addLink(to: URL(string: AppUrl.term), with: range1)
        let range2 : NSRange = str.range(of: Localized.privacyPolicy)
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        btnLogin.setTitle(Localized.signIn, for: UIControl.State.normal)
        ...
        btnFacebook.setTitle(Localized.connectWithFacebook, for: UIControl.State.normal)
        btnFacebook2.setTitle(Localized.connectWithFacebook, for: UIControl.State.normal)

        labelChooseLoginMethod.text = Localized.useYourEmail
       
        tfSignUpName.placeholder = Localized.username
        tfSignUpEmail.placeholder = Localized.email
        tfSignUpPass.placeholder = Localized.password
        tfSignUpRePass.placeholder = Localized.confirmPassword
        tfSignInEmail.placeholder = Localized.email
        tfSignInPass.placeholder = Localized.password
...
//        }
    ...
    fileprivate func setupSegmentioView() {
        let segments = [SegmentioItem(title: Localized.signIn, image: UIImage(named: "")),
                        SegmentioItem(title: Localized.signUp, image: UIImage(named: ""))]
    ...
    */
    ...
    func makeBackgroundSlideshow() {
        ...
        slideshow.slideshowInterval = 4.0 // 31/08/17 request change from Mr.Tho
    ...
    @IBAction func actionSignInWithFacebook(_ sender: Any) {
        ...
        manager.logIn(permissions: ["email"], from: self, handler: {(result: LoginManagerLoginResult?, error: Error?) in
            if error != nil || result == nil{
                ...
                Global.appDelegate.ebaseLoginWith(credential: credential, info: SignInInfo.makeFacebook(accessToken: tokenString))
            }
            else {
